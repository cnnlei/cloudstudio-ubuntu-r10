...
# 注意：必须先保存显式环境变量，再 source 旧配置，否则旧配置会覆盖 Manager 传入的新值。
# 空字符串不视为覆盖值，这样 Manager 留空时仍可沿用既有配置。
ENV_RELAY_PORT="${RELAY_PORT:-}"; ENV_SSH_PORT="${SSH_PORT:-}"; ENV_MODELS_FILE="${MODELS_FILE:-}"
ENV_RELAY_HOST="${RELAY_HOST:-}"; ENV_SSH_LISTEN="${SSH_LISTEN:-}"; ENV_UPSTREAM_BASE="${UPSTREAM_BASE:-}"
ENV_UPSTREAM_AUTH_MODE="${UPSTREAM_AUTH_MODE:-}"; ENV_UPSTREAM_TOKEN="${UPSTREAM_TOKEN:-}"
ENV_MODELS_MODE="${MODELS_MODE:-}"; ENV_STREAM_ADAPT="${STREAM_ADAPT:-}"; ENV_RELAY_KEY="${RELAY_KEY:-}"
...
OLD_UPSTREAM_TOKEN=""; OLD_MODELS_MODE=""; OLD_STREAM_ADAPT=""; OLD_RELAY_KEY=""
if [ -r "$CONF" ]; then
    ...
    OLD_MODELS_MODE="${MODELS_MODE:-}"; OLD_STREAM_ADAPT="${STREAM_ADAPT:-}"; OLD_RELAY_KEY="${RELAY_KEY:-}"
fi

RELAY_PORT="${ENV_RELAY_PORT:-${OLD_RELAY_PORT:-65530}}"
SSH_PORT="${ENV_SSH_PORT:-${OLD_SSH_PORT:-22}}"
MODELS_FILE="${ENV_MODELS_FILE:-${OLD_MODELS_FILE:-/workspace/models.json}}"
RELAY_HOST="${ENV_RELAY_HOST:-${OLD_RELAY_HOST:-127.0.0.1}}"
SSH_LISTEN="${ENV_SSH_LISTEN:-${OLD_SSH_LISTEN:-127.0.0.1}}"
UPSTREAM_BASE="${ENV_UPSTREAM_BASE:-${OLD_UPSTREAM_BASE:-http://auth.proxy/codebuddy/v2}}"
UPSTREAM_AUTH_MODE="${ENV_UPSTREAM_AUTH_MODE:-${OLD_UPSTREAM_AUTH_MODE:-bearer}}"
UPSTREAM_TOKEN="${ENV_UPSTREAM_TOKEN:-${OLD_UPSTREAM_TOKEN:-auto_proxy_token}}"
MODELS_MODE="${ENV_MODELS_MODE:-${OLD_MODELS_MODE:-local}}"
STREAM_ADAPT="${ENV_STREAM_ADAPT:-${OLD_STREAM_ADAPT:-true}}"
RELAY_KEY="${ENV_RELAY_KEY:-${OLD_RELAY_KEY:-}}"
